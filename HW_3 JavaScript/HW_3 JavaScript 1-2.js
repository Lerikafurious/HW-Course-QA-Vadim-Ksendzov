...
const findUser = users.filter(element => element.registrationDate == '09.10.2021' || element.registrationDate == '10.10.2021')
...
// Version 1
// const personCard = function(personId) {
//     for (let i = 0; i < personId.length; i++) {
//       for (let j = 0; j < personId.length;j++) {
//         if (personId[i].name == personId[j].name && i!= j ) {
//           personId.splice(j, 1)
//         }
//       }
//     }
...
// Version 2
const personCard = function(personId) {
    personId.forEach((item, index) => {
      personId.forEach((element, id) => {
        (item.name == element.name && index != id ) ? personId.splice(id, 1) : false
      })
    })
    return personId
}
console.log(personCard(personId))


// Task 2*** Реализуйте считывание JSONки из файла task2.json с помощью, например, модуля fs. 
// для дальнейшего использования в функции, описанной в задании

// Version 1
const jsonData = require('./task2.json')
console.log(jsonData)
console.log(personCard(jsonData))

// Version 2
// const fs = require('fs')
// const personData = JSON.parse(fs.readFileSync('./task2.json'))
// console.log(personCard(personData))
